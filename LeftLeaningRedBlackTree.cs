...
public class LeftLeaningRedBlackTree<TKey, TValue>
{
	...
	private bool IsMultiDictionary => null != _valueComparison;
	...
	{
		return Traverse(GetNodeForKey(key), (Node n) => 0 == _keyComparison(n.Key, key), (Node n) => n.Value);
	}

	public IEnumerable<TValue> GetValuesForAllKeys()
	{
		return Traverse(_rootNode, (Node n) => true, (Node n) => n.Value);
	}
	...
	private Node Add(Node node, TKey key, TValue value)
	{
		...
		int num = KeyAndValueComparison(key, value, node.Key, node.Value);
		if (num < 0)
		{
	...
	private Node Remove(Node node, TKey key, TValue value)
	{
		...
		else
		{
			...
			if (KeyAndValueComparison(key, value, node.Key, node.Value) == 0 && node.Right == null)
			{
				Count--;
				if (0 < node.Siblings)
				{
			...
			if (node.Right != null)
			{
				...
				if (KeyAndValueComparison(key, value, node.Key, node.Value) == 0)
				{
					Count--;
					if (0 < node.Siblings)
					{
	...
	private Node GetNodeForKey(TKey key)
	{
		...
		while (node != null)
		{
			int num = _keyComparison(key, node.Key);
			if (num < 0)
			{
				node = node.Left;
				continue;
	...
	{
		int num = _keyComparison(leftKey, rightKey);
		if (num == 0 && _valueComparison != null)
		{
			num = _valueComparison(leftValue, rightValue);
		}
		return num;
	}
}
