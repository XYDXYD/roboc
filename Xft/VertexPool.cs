...
namespace Xft
{
	public class VertexPool
	{
		...
		{
			Vertices = (Vector3[])new Vector3[4];
			UVs = (Vector2[])new Vector2[4];
			UVs2 = (Vector2[])new Vector2[4];
			Colors = (Color[])new Color[4];
			Indices = new int[6];
			VertexTotal = 4;
			IndexTotal = 6;
		...
		public void EnlargeArrays(int count, int icount)
		{
			...
			Vertices = (Vector3[])new Vector3[Vertices.Length + count];
			vertices.CopyTo(Vertices, 0);
			Vector2[] uVs = UVs;
			UVs = (Vector2[])new Vector2[UVs.Length + count];
			uVs.CopyTo(UVs, 0);
			Vector2[] uVs2 = UVs2;
			UVs2 = (Vector2[])new Vector2[UVs2.Length + count];
			uVs2.CopyTo(UVs2, 0);
			InitDefaultShaderParam(UVs2);
			Color[] colors = Colors;
			Colors = (Color[])new Color[Colors.Length + count];
