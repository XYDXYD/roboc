...
{
	internal sealed class SwitchToCampaignCommand : IInjectableCommand<SwitchSinglePlayerCampaignWorldDependency>, ICommand
	{
		private SwitchSinglePlayerCampaignWorldDependency _dependency;
		...
		public ICommand Inject(SwitchSinglePlayerCampaignWorldDependency dependency)
		{
		...
		private void SwitchToPlanet(ReadOnlyDictionary<string, PlayerDataDependency> expectedPlayers, AvatarInfo avatarInfo, ReadOnlyDictionary<string, ClanInfo> clanInfos)
		{
			...
			ILogPlayerEnteredCampaignRequest logPlayerEnteredCampaignRequest = analyticsRequestFactory.Create<ILogPlayerEnteredCampaignRequest>();
			logPlayerEnteredCampaignRequest.Execute();
			BattleParametersData battleParameters = new BattleParametersData("localhost", 0, string.Empty, new GameModeKey(GameModeType.Campaign), DateTime.UtcNow, null, null, "<singleplayer_guid>");
			...
			worldSwitching.SwitchToPlanetSinglePlayerCampaign(_dependency);
		}
	}
}
